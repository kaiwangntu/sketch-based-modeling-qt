...
float MyMath::dotProduct( float vec1[3], float vec2[3])
{
	float dotp = 0;
	for( int i = 0; i < 3; i ++)
		dotp += (vec1[ i ] * vec2[ i ]);
...
void MyMath::getVec(float pt1[3], float pt2[3], float vec[3])
{
	for( int i = 0; i < 3; i ++)
		vec[ i ] = pt2[ i ] - pt1[ i ];
}
...
void MyMath::getEndPtOfSeg(float ept[ 3 ], float vec[ 3 ], float resultpt[ 3 ])
{
	for( int i = 0; i< 3; i ++)
...
void MyMath::getrelativepos(float pt[3], float center[3], float len)
{
	for( int i = 0; i < 3; i ++)
	{
...
void MyMath::stretchVec( float vec[ 3 ], float ratio[ 3 ] )
{
	for( int i = 0; i < 3; i++)
		vec[ i ] *= ratio[ 3 ];
}

void MyMath::stretchVec( float vec[ 3 ], float ratio)
{
	for( int i = 0; i < 3; i++)
		vec[ i ] *= ratio;
}
