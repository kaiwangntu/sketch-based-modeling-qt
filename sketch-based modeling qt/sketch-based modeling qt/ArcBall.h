...
class AutoPlaneRotation
{
public:
	//precompute
	//iStep: number of rotation to reach the end
	bool PreCompute(Plane_3 plane,Point_3* PlaneBoundaryPoints,GLdouble* modelview,int iStep,Matrix3fT& CurrentRotIn);
	//rotate in each step
	bool Rotate(Matrix4fT& CurrentTransformMatrix,Matrix3fT& CurrentRotIn);

protected:
	//how many steps and what's the current step
	int iTotalStep,iStepCount;
	//rotation axis in each step
	Vector_3 RotateAxis;
	//angle to rotate in each step
	double dStepAngle;
	//Last rotation matrix before any rotation
	Matrix3fT LastRot;
};
