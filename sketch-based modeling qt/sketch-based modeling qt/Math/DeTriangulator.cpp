...

{
...
#ifndef TRILIBRARY
  strcpy(objfname, argv[2]);
  strcat(objfname, ".obj");
  objfp = fopen(objfname, "wt") ;
  ...
  fprintf(objfp, "# vertices: %d  triangles: %d\n", outvertices, m.triangles.items);
  ...
  while (vertexloop != (vertex) NULL) 
  {
	if (!b.jettison || (((int *) (vertexloop))[m.vertexmarkindex + 1] != UNDEADVERTEX))
	{
		/* Vertex number, x and y coordinates. */
		fprintf(objfp, "v %f %f 0.0\n", vertexloop[0], vertexloop[1]);
  ...
  while (triangleloop.tri != (triangle *) NULL) 
  {
	...
	{
      /* Triangle number, indices for three vertices. */
      fprintf(objfp, "f %d %d %d\n", ((int *) (p1))[m.vertexmarkindex],
		  ((int *) (p2))[m.vertexmarkindex],
		  ((int *) (p3))[m.vertexmarkindex]);
    }
